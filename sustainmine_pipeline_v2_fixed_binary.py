...
class SustainMineDataPipelineV2:
    def __init__(
        ...
        if "Date" not in self.sensor_df.columns:
            raise ValueError("Sensor CSV must contain a 'Date' column")

        self.sensor_df["Date"] = pd.to_datetime(self.sensor_df["Date"]).dt.strftime("%Y-%m-%d")

        if self.label_column not in self.sensor_df.columns and "pollution_label" in self.sensor_df.columns:
            ...
            samples.append(
                {
                    ...
                    "label": sensor_row.get(self.label_column),
    ...
    def create_summary(self) -> Dict:
        ...
        if self.label_column in self.sensor_df.columns:
            counts = self.sensor_df[self.label_column].value_counts().sort_index().to_dict()
            label_distribution = {int(k): int(v) for k, v in counts.items()}
...
if __name__ == "__main__":
    pipeline = SustainMineDataPipelineV2("sensor_data_cleaned_with_binary_labels.csv")
