...
- Binary classification: 0 = low, 1 = polluted
"""
...
class ClassificationHead(nn.Module):
    def __init__(self, embed_dim: int = 384, num_classes: int = 2):
        super().__init__()
...
class SustainMineModelV2(nn.Module):
    def __init__(self, img_size: int = 224, patch_size: int = 16, in_channels: int = 6,
                 ...
                 num_heads: int = 6, num_classes: int = 2, num_forecast_steps: int = 3,
...
def compute_multi_task_loss(outputs: Dict[str, torch.Tensor], labels: torch.Tensor,
                            ...
                            beta: float = 0.5, class_weights: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
    class_loss = F.cross_entropy(outputs["classification"], labels, weight=class_weights)
